...
public abstract class AbstractEndpoint<S> {
    ...
    /**
     * Running state of the endpoint. endpoint的运行状态
     */
    protected volatile boolean running = false;


    /**
     * Will be set to true whenever the endpoint is paused.endpoint的暂停状态
     */
    ...
    /**
     * counter for nr of connections handled by an endpoint 计数当前endpoint处理的connections
     */
    ...
    /**
     * Acceptor thread count.acceptor线程从已完成连接的队列中取出连接调用accept方法，这里是指明有
     * 多少个线程做这件事。默认为1
     */
    ...
    /**
     * tomcat对连接数的控制，如果acceptor在准备从队列中accept连接时，发现当前连接数超出了maxConnections则会阻塞。
     * 但是并不会阻止新连接的建立，新连接的建立和TCP层的参数相关，当连接数超过maxConnections时，仍然可以建立连接
     * 只是存放在已完成连接队列中，但是不会有acceptor去accept，和做后续的读写处理。
     */
     ...
     * is 100.TCP三次握手建立连接后放入这个队列，已完成TCP连接的队列，serverSocketChannle就是
     * 从这个队列中取出建立连接的请求accept。
     */
    ...
    /**
     * Maximum amount of worker threads.处理IO事件的最大工作线程数。
     */
    ...
    public void createExecutor() {
        ...
        //核心线程数默认是10个，最大线程数默认200，这里的TaskQueue进行了相应包装。在核心线程数满了后会优先创建新线程来处理，而不是将任务放入队列。
    ...
    public boolean processSocket(SocketWrapperBase<S> socketWrapper,
        ...
        try {
            ...
            //取SocketProcessorBase包装处理事件。还是cache那一套。
     ...
     * 这里没有维护状态和状态转换的合法性检查，仅仅确保bind/unbind在正确的位置调用，调用代码应该维护
     */
     ...
     * @return the acceptor 实现Acceptor的钩子函数，允许子类做具体实现
     */
    ...
    protected LimitLatch initializeConnectionLatch() {
        ...
        if (connectionLimitLatch==null) {//初始化为最大连接数
