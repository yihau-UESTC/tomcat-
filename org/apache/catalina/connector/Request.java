...
public class Request implements org.apache.catalina.servlet4preview.http.HttpServletRequest {
    ...
    // ------------------------------------------------------ Protected Methods
    //servlet中获取servlet的逻辑
    protected Session doGetSession(boolean create) {

        // There cannot be a session if no context has been assigned yet 首先获取绑定的context，即当前web app容器
        Context context = getContext();
        if (context == null) {
            return (null);
        }
        ...
        if ((session != null) && !session.isValid()) {//session不合法置null
            session = null;
        }
        if (session != null) {
            return (session);
        ...
        Manager manager = context.getManager();//manager用来管理session，只有context容器中有使用，session是context相关的。
        if (manager == null) {
            return (null);      // Sessions are not supported
        }
        if (requestedSessionId != null) {//这个requestedSessionId是从http request中解析出来的，即请求携带的sessionid
            try {
                session = manager.findSession(requestedSessionId);//按id查找。
            } catch (IOException e) {
                session = null;
            }
            if ((session != null) && !session.isValid()) {
        ...
        // circumstances.极少情况下会重新使用客户端提供的session ID
        ...
        if (requestedSessionSSL) {
             ...
             * 在同一主机的不同web中使用相同的sessionID，这时portlet的典型实现，只有在session
             * 通过cookies标记，而且cookie的path必须为“/”。这时需要核对这个sessionID是否存在
             * host上的其他web中。
             */
            if (context.getValidateClientProvidedNewSessionId()) {//判断客户端提供的sessionid是否合法
                boolean found = false;
                for (Container container : getHost().findChildren()) {//遍历host所有context，寻找session
        ...
        // Creating a new session cookie based on that session 关联sessionid 和cookie
        if (session != null
                && context.getServletContext()
                        .getEffectiveSessionTrackingModes()
                        .contains(SessionTrackingMode.COOKIE)) {
