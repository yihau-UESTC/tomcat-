...
public class CoyoteAdapter implements Adapter {
    ...
    public void service(org.apache.coyote.Request req, org.apache.coyote.Response res)
        ...
        if (request == null) {
            // Create objects创建req和resp
            request = connector.createRequest();
            request.setCoyoteRequest(req);
            response = connector.createResponse();
            response.setCoyoteResponse(res);

            // Link objects互相关联req和resp
            ...
            // Set query string encoding设置查询字符串编码
        ...
        try {
            ...
            // request parameters这里面会进行URL到对应container的映射
            postParseSuccess = postParseRequest(req, request, res, response);
            if (postParseSuccess) {
                //check valves if we support async
                request.setAsyncSupported(
                        connector.getService().getContainer().getPipeline().isAsyncSupported());
                // Calling the container 调用容器中的pipeline处理req和resp
                connector.getService().getContainer().getPipeline().getFirst().invoke(
                        request, response);
            }
            if (request.isAsync()) {
     ...
     * 在解析过请求头后执行一些必须的处理，使得req/resp对 可以传递到容器管道的起点处理
    ...
    protected boolean postParseRequest(org.apache.coyote.Request req, Request request,
        ...
        // processor hasn't set it, use the settings from the connector AJP协议在此不为null
        if (req.scheme().isNull()) {
            // Use connector scheme and secure configuration, (defaults to
            // "http" and false respectively)使用connect的scheme(例如http)和安全配置
        ...
        // Override if the proxyPort/proxyHost are set服务器内部代理重定向
        ...
        // Request mapping.请求mapping
        MessageBytes serverName;
        if (connector.getUseIPVHosts()) {//是否开启了基于IP的虚拟主机
        ...
        while (mapRequired) {
            // This will map the the latest version by default在Service的全局路由mapper中查找Host、URI对应的Host、Contex、Wrapper容器
            ...
            // so no context could be mapped.这里可能是因为没有部署一个根context或者是不合法的context请求
            ...
            if (request.getServletContext().getEffectiveSessionTrackingModes()
                    .contains(SessionTrackingMode.URL)) {//session追踪模式COOKIE/URL/SSL，这里看是否支持URL形式

                // Get the session ID if there was one从path中尝试获取session ID
                sessionID = request.getPathParameter(
                        SessionConfig.getSessionUriParamName(
                                request.getContext()));
                if (sessionID != null) {//设置sessionID
                    request.setRequestedSessionId(sessionID);
                    request.setRequestedSessionURL(true);
                }
            }

            // Look for session ID in cookies and SSL session分别从cookies和SsL中解析sessionid
            ...
            if (version != null && request.getContext() == versionContext) {
                ...
                if (contexts != null && sessionID != null) {
                    // Find the context associated with the session找出context相关的session，contex中有个管理session的pool
            ...
            if (!mapRequired && request.getContext().getPaused()) {
                ...
                // point.发现contex暂停需要等待，可能是一些wrapper正在注册
                try {
                    Thread.sleep(1000);
        ...
        if (!redirectPathMB.isNull()) {
            ...
            response.sendRedirect(redirectPath);//内部重定向
            request.getContext().logAccess(request, response, 0, true);
            return false;
        }

        // Filter trace method处理TRACE回显请求
        if (!connector.getAllowTrace()
