...
public class Connector extends LifecycleMBeanBase  {
    ...
    public Connector(String protocol) {
        ...
        try {
            Class<?> clazz = Class.forName(protocolHandlerClassName);//根据类名反射创建要用的HTTP协议IO处理类型
        ...
        if (Globals.STRICT_SERVLET_COMPLIANCE) {//设置字符集
    ...
    /**
     * Coyote protocol handler.接收socket请求，并将其解析为HTTP请求对象，具体实现可以是
     * NIO或BIO(废除),AJP模式，其中包含了一些endpoint来处理具体的接收socket、读写socket的底层操作
     */
    protected final ProtocolHandler protocolHandler;


    /**
     * Coyote adapter.处理HTTP请求对象，完成Engine的value到用到servlet的valve
     */
    ...
    protected void initInternal() throws LifecycleException {
        ...
        // Initialize adapter初始化coyote的适配器，委派request处理过程
        ...
        try {
            protocolHandler.init();//解析server.xml的时候，构建connector，会创建protocolHandler
        } catch (Exception e) {
