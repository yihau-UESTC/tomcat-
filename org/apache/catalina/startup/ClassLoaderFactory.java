...
public final class ClassLoaderFactory {
    ...
    public static ClassLoader createClassLoader(List<Repository> repositories,
        ...
        // Construct the "class path" for this class loader
        //包装repositories中所有的路径为URL为类加载器构建类路径
        Set<URL> set = new LinkedHashSet<>();

        if (repositories != null) {
            for (Repository repository : repositories)  {
                if (repository.getType() == RepositoryType.URL) {//URL类型的处理
                    ...
                    //将文件目录封装为URL类型file:/E:/tomcat/apache-tomcat-8.5.39/apache-tomcat-8.5.39/lib/
        ...
        //这个方法返回的兑现够可以获取特权，绕过权限检查，java安全相关的知识，之后再补充，这里应该是对URLClassLoader的包装。
        return AccessController.doPrivileged(
                new PrivilegedAction<URLClassLoader>() {
                    @Override
                    public URLClassLoader run() {
                        if (parent == null)
                            //创建一个URLClassLoader，继承于ClassLoader
                            return new URLClassLoader(array);
                        else
                            return new URLClassLoader(array, parent);
                    }
