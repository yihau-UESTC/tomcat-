...
public class StandardService extends LifecycleMBeanBase implements Service {
    ...
    /**
     * 这里的executors作用？？？这里的Executor是在server.xml中配置的，解析的时候会创建对象。
     * 如果选择在server.xml中配置server中组件共享的线程池，那么在parse server.xml的时候会
     * 将线程池创建在这里。然后如果在http endpoint中配置使用了该共享线程池，那么启动该endpoint时
     * 变不需要创建自己的worker线程池，直接使用这个线程池即可。但是acceptor和poller线程还是需要创建的。
     */
    ...
    /**
     * 全局路由Mapper.过去的版本mapper组合于Connector中，现在Service中。
     */
    protected final Mapper mapper = new Mapper();


    /**
     * Mapper listener. 监听器，在听到容器添加移除事件时维护mapper的映射关系。
     */
    ...
    protected void startInternal() throws LifecycleException {
        ...
        // Start our defined Container first 启动引擎，在这当中会继续启动Host，进而会部署webapps下面的各个web应用
        if (engine != null) {
            synchronized (engine) {
                engine.start();
            }
        ...
        // Start our defined Connectors second 启动connector
        synchronized (connectorsLock) {
    ...
    protected void initInternal() throws LifecycleException {
        ...
        // Initialize any Executors初始化线程执行器
        ...
        // Initialize our defined Connectors初始化connector
        synchronized (connectorsLock) {
            for (Connector connector : connectors) {
                try {
                    connector.init();
