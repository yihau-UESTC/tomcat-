...
public final class StandardServer extends LifecycleMBeanBase implements Server {
    ...
    protected void startInternal() throws LifecycleException {
        ...
        // Start our defined Services //开启server中定义的service
        synchronized (servicesLock) {
            for (int i = 0; i < services.length; i++) {
                services[i].start();
            }
    ...
    protected void initInternal() throws LifecycleException {
                        ...
                        if (url.getProtocol().equals("file")) {
                            try {
                                ...
                                if (f.isFile() &&
                                    ...
                                    ExtensionValidator.addSystemResource(f);//将jar包的MANIFEST添加到容器的MANIFEST中
                                }
        ...
        // Initialize our defined Services初始化server中的service，这过程中还会初始化service中使用的Engine和connector
        for (int i = 0; i < services.length; i++) {
            services[i].init();
        }
    }
