...
public class Http11Processor extends AbstractProcessor {
    ...
    //调用adapter的service方法将其提交到catalina容器中处理
    @Override
    public SocketState service(SocketWrapperBase<?> socketWrapper)
        ...
        // Setting up the I/O 初始化读写buffer
        ...
        //processor不处于故障状态&&keepAlive&&不是异步请求模式&&不升级协议&&不是sendFile&&通信端点未暂停工作
        while (!getErrorState().isError() && keepAlive && !isAsync() && upgradeToken == null &&
                sendfileState == SendfileState.DONE && !endpoint.isPaused()) {

            // Parsing the request header解析请求行
            try {
                if (!inputBuffer.parseRequestLine(keptAlive)) {
                    if (inputBuffer.getParsingRequestLinePhase() == -1) {//-1代表是HTTP2.0需要升级处理
                        return SocketState.UPGRADING;
                    } else if (handleIncompleteRequestLineRead()) {//没有完成读请求，break
                        break;
                    }
                }

                if (endpoint.isPaused()) {//如果endpoint暂停工作了，直接response503
                    // 503 - Service unavailable
                    response.setStatus(503);
                    setErrorState(ErrorState.CLOSE_CLEAN, null);//更新错误状态
                } else {
                    keptAlive = true;
                    // Set this every time in case limit has been changed via JMX设置request中请求头的最大个数
                    request.getMimeHeaders().setLimit(endpoint.getMaxHeaderCount());
                    if (!inputBuffer.parseHeaders()) {
                        // We've read part of the request, don't recycle it
                        // instead associate it with the socket返回false的时候说明我们读取的部分请求，不要回收读到的，改变状态，告诉socket还没读完
                        openSocket = true;
                        readComplete = false;
                        break;
                    }
            ...
            // Has an upgrade been requested?判断是否需要协议升级，tomcat支持http2.0和websocket两种协议升级
            ...
            if (getErrorState().isIoAllowed()) {
                // Setting up filters, and parse some request headers更新请求的阶段。
                rp.setStage(org.apache.coyote.Constants.STAGE_PREPARE);
                try {
                    prepareRequest();//核对HTTP的头部信息
                } catch (Throwable t) {
                    ExceptionUtils.handleThrowable(t);
                    if (log.isDebugEnabled()) {
            ...
            // Process the request in the adapter 调用adapter的service方法将其提交到catalina容器处理
            if (getErrorState().isIoAllowed()) {
                try {
    ...
    private void prepareRequest() {
        ...
        if (protocolMB.equals(Constants.HTTP_11)) {
            ...
            // Unsupported protocol不支持的协议返回505
            http11 = false;
        ...
        if (connectionValueMB != null) {//ByteChunk 一种byte数组
            ByteChunk connectionValueBC = connectionValueMB.getByteChunk();
            if (findBytes(connectionValueBC, Constants.CLOSE_BYTES) != -1) {//close表示本次请求后关闭连接
                keepAlive = false;
            } else if (findBytes(connectionValueBC,
                                 Constants.KEEPALIVE_BYTES) != -1) {//keepalive表示保持连接
                keepAlive = true;
            }
        }

        if (http11) {//核对expect头部
        ...
        // Check user-agent header核对是否限制了user-agent头部，如果有检查是否按照规定
        ...
        parseHost(hostValueMB);//分离主机名 和端口号
