...
public class Http11InputBuffer implements InputBuffer, ApplicationBufferHandler {
     ...
     * start of the body.记录buffer中header的末端字符位置，初始化为0
     */
    private int end;
     ...
     * leading blank lines.请求的总体长度不能超过这个值
     */
    ...
    boolean parseRequestLine(boolean keptAlive) throws IOException {
        ...
        // check state 核对是否解析过了，这是用于非阻塞处理时记录状态用。
        if (!parsingRequestLine) {
            return true;
        }
        //
        // Skipping blank lines parsingRequestLinePhase初始值为0
        //
        if (parsingRequestLinePhase < 2) {
            byte chr = 0;
            do {

                // Read new bytes if needed读取请求头的数据
                if (byteBuffer.position() >= byteBuffer.limit()) {
                    ...
                    if (!fill(false)) {
                        // A read is pending, so no longer in initial state 读数据需要等待，改变parsingRequestLinePhase的阶段。
                        parsingRequestLinePhase = 1;
                        return false;
                    }
                    // At least one byte of the request has been received.
                    // Switch to the socket timeout.设置wrapper的read timeout
                ...
                if (!keptAlive && byteBuffer.position() == 0 && byteBuffer.limit() >= CLIENT_PREFACE_START.length - 1) {
                    ...
                    if (prefaceMatch) {
                        // HTTP/2 preface matched 如果是HTTP2.0，修改请求行阶段状态，返回
                        parsingRequestLinePhase = -1;
                        return false;
                    }
                }
                // Set the start time once we start reading data (even if it is
                // just skipping blank lines)设置request的开始时间
                if (request.getStartTime() < 0) {
                    request.setStartTime(System.currentTimeMillis());
                }
                chr = byteBuffer.get();
            } while ((chr == Constants.CR) || (chr == Constants.LF));//跳过/r 和/n头部空白行
            byteBuffer.position(byteBuffer.position() - 1);//恢复buffer的位置

            parsingRequestLineStart = byteBuffer.position();
            parsingRequestLinePhase = 2;//设置解析请求行阶段为2
        ...
        if (parsingRequestLinePhase == 2) {
            //阶段2解析出请求方法名 GET POST
            // Reading the method name
            // Method name is a token
            boolean space = false;
            while (!space) {
                // Read new bytes if needed 如果当前pos已经大于等于lim，说明数据已经用完，需要再读一些
                if (byteBuffer.position() >= byteBuffer.limit()) {
                ...
                if (chr == Constants.SP || chr == Constants.HT) {//检查空格
                    space = true;
        ...
        if (parsingRequestLinePhase == 3) {//阶段3检查是否有多个空格
        ...
        if (parsingRequestLinePhase == 5) {
            // Spec says single SP but also be tolerant of multiple and/or HT，URI和HTTP版本之间也可能有多个空格
            boolean space = true;
            while (space) {
                // Read new bytes if needed
                if (byteBuffer.position() >= byteBuffer.limit()) {
    ...
    /**
     * Parse the HTTP headers.解析HTTP头部字段
     */
    boolean parseHeaders() throws IOException {
        ...
        } while (status == HeaderParseStatus.HAVE_MORE_HEADERS);//当还有头部字段未解析的时候继续。
     ...
     * 尝试读一些数据到input buffer中。如果读到数据返回true，否则返回false
    ...
    private boolean fill(boolean block) throws IOException {
        ...
        if (parsingHeader) {
            if (byteBuffer.limit() >= headerBufferSize) {//请求行长度过长，抛出异常
                if (parsingRequestLine) {
        ...
        } else {//设置buffer的limit和position
            byteBuffer.limit(end).position(end);
        }

        byteBuffer.mark();//记录buffer当前pos
        if (byteBuffer.position() < byteBuffer.limit()) {//如果当前pos<lim,设置pos为lim，继续填充buffer
            byteBuffer.position(byteBuffer.limit());
        }
        byteBuffer.limit(byteBuffer.capacity());//然后将lim扩充到cap
