...
public abstract class AbstractProtocol<S> implements ProtocolHandler,
    ...
    //主要用于为连接选择一个合适的Processor来处理请求。
    protected static class ConnectionHandler<S> implements AbstractEndpoint.Handler<S> {

        private final AbstractProtocol<S> proto;
        private final RequestGroupInfo global = new RequestGroupInfo();
        private final AtomicLong registerCount = new AtomicLong(0);
        //缓存S(socket)和其处理processor
        ...
        public SocketState process(SocketWrapperBase<S> wrapper, SocketEvent status) {
            ...
            try {
                //如果processor不存在，根据协商协议构造一个processor
                if (processor == null) {
                ...
                //如果不存在协商协议，则从回收队列中获取一个processor来使用，
                if (processor == null) {
                ...
                }//如果回收队列中也没有，那么由当前的协议创建一个processor
                if (processor == null) {
                    processor = getProtocol().createProcessor();
                    register(processor);//将该processor注册到MBServer
                }
                //ssl相关
                processor.setSslSupport(
                        wrapper.getSslSupport(getProtocol().getClientCertProvider()));

                // Associate the processor with the connection，将socket和processor的键值对放入缓存
